...
Try {
    Invoke-WebRequest -Uri "http://localhost:5064/aspnet-core1" -Method Get -MaximumRedirection 0 -ErrorAction SilentlyContinue | Out-Null
...
$out1 = & $ab -n 1000 -c 10 "http://localhost:5064/aspnet-core1"
...
*Nota: Al enviar la petición mediante \`ab\` sin el token AntiForgery, retorna \`400 Bad Request\`, lo cual es captado como \`Non-2xx response\`. El rate limiting fue aplicado antes de esta validación.*
...
Set-Content -Path docs\benchmark-after.md -Value $md -Encoding UTF8
Write-Host "Benchmark completed. Report written to docs\benchmark-after.md"
